    ...
    <div>
      <Card title="상품목록" noborder>
        <div class="buttons-container">
          <!-- 상품등록 모달 -->
          <Modal title="상품등록" label="상품등록" labelClass="btn inline-flex justify-center btn-dark btn-sm mr-3" ref="modal1" @closed="resetModalData">
            <div class="text-base text-slate-600 dark:text-slate-300">
              ...
              <Textinput label="정가" type="number" name="listPrice" v-model="addModalData.listPrice" placeholder="상품의 정가를 입력하세요." class="mb-2" />
              <Textinput label="재고" type="number" name="productStock" v-model="addModalData.productStock" placeholder="재고를 입력하세요." class="mb-2" />
              <Textinput label="브랜드" type="text" name="brand" v-model="addModalData.brand" placeholder="브랜드를 입력하세요" class="mb-2" />
              <label class="ltr:inline-block rtl:block input-label">상품이미지</label><br>
              <input type="file" id="imageUpload" @change="handleAddImageUpload" accept="image/jpeg, image/png, image/gif" class="mt-2 text-base text-slate-600 dark:text-slate-300" />
              <img v-if="addModalData.previewImage" :src="addModalData.previewImage" alt="대표 이미지 미리보기" style="max-width: 100px">
            </div>
          ...
          <!-- 상품삭제 버튼 -->
          <button class="btn inline-flex justify-center btn-outline-dark btn-sm" @click="deleteSelectedProducts"><span>상품삭제</span></button>
          ...
          <Modal title="상품 정보 수정" ref="editModal" :showButtons="false" @closed="resetEditModalData">
            <div class="text-base text-slate-600 dark:text-slate-300">
              ...
              <Textinput label="재고" type="number" name="editproductstock" v-model="editModalData.productStock" class="mb-2" />
              <Textinput label="브랜드" type="text" name="brand" v-model="editModalData.brand" class="mb-2" />
              <label class="ltr:inline-block rtl:block input-label">상품이미지</label><br>
              <input type="file" id="imageUpload" @change="handleEditImageUpload" accept="image/jpeg, image/png, image/gif" class="mt-2 text-base text-slate-600 dark:text-slate-300" />
              <img v-if="editModalData.previewImage" :src="editModalData.previewImage" alt="대표 이미지 미리보기" style="max-width: 100px">
            </div>
            <template v-slot:footer>
        ...
        <div class="-mx-6">
          <vue-good-table :columns="columns" styleClass="vgt-table centered lesspadding2 table-head" :rows="products" :pagination-options="{ enabled: true, perPage: perpage }" :sort-options="{ enabled: false }">
            <template v-slot:table-row="props">
              ...
              <span v-if="props.column.field === 'customer'" class="flex">
                <img v-for="entry in props.row.customer" :key="entry.name" :src="entry.image" :alt="entry.name" class="object-cover w-full h-full rounded-full" style="width: 24px; margin-right: 5px;" />
              </span>
            </template>
            <template #pagination-bottom="props">
              <div class="py-4 px-3 flex justify-center">
                <Pagination :total="50" :current="current" :per-page="perpage" :pageRange="pageRange" @page-changed="current = $event" :pageChanged="props.pageChanged" :perPageChanged="props.perPageChanged">
                </Pagination>
              </div>
            </template>
          </vue-good-table>
